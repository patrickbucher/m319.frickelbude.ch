+++
date = '2026-01-04T20:45:46+01:00'
title = 'Python'
weight = 3
+++
...
1. Sie können Mehrfachverzweigungen mit `if`, `elif` und `else` formulieren.
1. Sie können `if`/`else` wie einen ternären Operator für Ausdrücke verwenden.
1. Sie können Bedingungen für den ternären Operator, `if` und `elif` formulieren.

## Mehrfachverzweigungen

1. Sie können Mehrfachverzweigungen mit `match`/`case` formulieren.
1. Sie können dazu eine Standardbehandlung mit dem Fall `_` formulieren.
1. Sie können beurteilen, ob für eine gegebene Aufgabenstellung der Einsatz von `match`/`case` oder `if`/`elif`/`else` sinnvoller ist.
1. Sie können bestehende (und dazu passende) `if`/`elif`/`else`-Kontrukte in `match`/`case`-Konstrukte umschreiben und umgekehrt.
...
1. Sie können mit `for`/`in` über die Elemente einer Liste iterieren.
1. Sie können mit `for`/`in` über einen durch `range()` definierten Zahlenbereich iterieren.
